...
**Invoke this skill for the first edit to a given `~/personal/<topic>.md` file in a session** — it loads the doc's current structure and the memory-hygiene/Now/Coming Up/Decide/Log conventions that keep it from drifting. Once you've done that for a doc, later patches to the *same* doc within the *same* session can go through direct Edit/Write calls, as long as they still follow those conventions (right section, memory-hygiene check, commit each change) — no need to re-invoke for every small update. A new session, or a doc you haven't touched yet this session, starts back at the gate below.
