...
- **Dollar signs get eaten by bash in any inline argument, not just message/body flags.** `$8M` → `M`, `$120` disappears entirely — bash interprets `$X` as variable interpolation inside unquoted or double-quoted inline strings, regardless of tool or argument position. Whenever text contains a `$` amount, write it to a file first and pass it via `"$(cat file)"` or a heredoc instead of inlining it directly. Applies everywhere, not just `--body`/`-m` flags — hit this in a `git commit -m`, a `gog draft create --body`, and a plain `mv`/`cp` filename argument (`mv file.txt "Receipt ($14975.61).eml"` silently drops everything after `$14975` since bash treats it as an empty variable reference). For filenames specifically, either single-quote the literal `$` (`'...($14975.61)...'`) or substitute a non-`$` separator (e.g. spell out "USD") to sidestep the interpolation risk entirely.
